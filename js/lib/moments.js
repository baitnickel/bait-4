...
export class Moment {
    ...
    static parseDate(dateString) {
        ...
        if (!MomentString.test(dateString))
            return segments;
        ...
        if (components.length == 1)
            ...
            if (lastComponent.length > 2)
                components.reverse(); /** Y is last, reverse array to force YM or YMD */
            segments.push(Number(components[0])); /** year */
            ...
            if (segments[1] < 0 || segments[1] >= 12)
                ...
                if (!Moment.validDay(segments[0], segments[1] + 1, segments[2]))
    ...
    formatted() {
        if (this.date == null)
            ...
            if (this.precision == 1)
                ...
                format = 10; /** year and month */
            else if (this.precision == 3)
                format = 9; /** year, month, and day */
