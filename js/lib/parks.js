...
export class Park {
        ...
        if (startDate && endDate) {
            ...
            for (const cell of cells) {
                if (siteMap.has(cell.siteNumber)) {
                    ...
                    siteMap.set(cell.siteNumber, [cell]);
            }
            /**
            ...
            for (const siteNumber of siteNumbers) {
                ...
                for (const cell of siteCells) {
                    if (cell.offset > nextColumn) {
                        ...
                        let attribute = '';
                        if (unreservedDays > 1)
                            attribute = `colSpan: ${unreservedDays}`;
                        table.addCell('', attribute);
                        nextColumn += unreservedDays;
                    }
                    let camperName = (purchaserView) ? cell.purchaserAccount : cell.occupantNames;
                    if (cell.modified >= ModifiedSymbols.length)
                        camperName += ` ${ModifiedSymbols[ModifiedSymbols.length - 1]}`;
                    else if (cell.modified > 0)
                        camperName += ` ${ModifiedSymbols[cell.modified - 1]}`;
                    const attributes = [];
                    if (cell.reserved > 1)
                        attributes.push(`colSpan: ${cell.reserved}`);
     ...
     */
    addDebts(share, payments, debts) {
        const hostKeys = Array.from(payments.keys());
        const adjustments = new Map(); /** key is host, value is payments in cents */
        const creditors = []; /** hosts who have paid more than their share */
        const debtors = []; /** hosts who have paid less than their share */
        /**
        ...
        for (const hostKey of hostKeys) {
            ...
            if (adjustment > 0)
                creditors.push(hostKey);
            else if (adjustment < 0)
                debtors.push(hostKey);
        }
        /**
        ...
        for (const debtor of debtors) {
            ...
            for (const creditor of creditors) {
                if (adjustments.get(creditor) == 0)
                    continue; /** skip creditor who's been "paid off" */
    ...
    loadHosts(hostsDBData) {
        ...
        for (const key of Array.from(hostsDBData.keys())) {
            ...
            if (host)
                hosts.set(key.toUpperCase(), { name: host.name, color: host.color });
        }
        return hosts;
    }
