...
export class Range {
    ...
    tally(values = []) {
        ...
        if (maximumResult < this.limit)
            tally = '-'; /* can't cover range--underflow */
        else {
            ...
            if (sum > largestValidSum)
                tally = '+'; /* overflows range */
            else
                tally = sum % this.limit;
        }
        return tally;
    }
    /**
...
export class NumberRange {
    ...
    recalibrate(number, otherNumberRange, wrap = false) {
        ...
        if (wrap)
            newNumber = this.wrapRange(newNumber);
        return newNumber;
    }
     ...
     */
    wrapRange(number) {
        if (number < this.first)
            number = this.first; // Math.abs(number);
        else if (number >= (Math.floor(this.size / 2)))
