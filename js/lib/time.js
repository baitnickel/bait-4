...
export class Time extends Date {
    ...
    valid;
    precision; /** YearPrecision, YearMonthPrecision, YearMonthDayPrecision, or FullPrecision */
    estimate; /** string values ending with '?' are estimates */
    constructor(parameter = null, monthIndex = null, day = null, hours = 0, minutes = 0, seconds = 0, milliseconds = 0) {
        ...
        if (parameter === null)
            ...
            if (precision != FullPrecision)
                parameter += 'T00:00:00.000'; /** specify time to convert UTC to local */
            super(parameter);
        }
        if (precision == FullPrecision &&
            this.getHours() == 0 &&
    ...
    interval(other, granularity) {
        ...
        if (granularity == Years) {
            ...
            if (later.year == earlier.year)
                interval = (laterTime.ordinalDay() - earlierTime.ordinalDay()) / solarDays;
            ...
            if (granularity == Minutes)
                divisor = Time.msPerMinute;
            else if (granularity == Hours)
                divisor = Time.msPerHour;
            else if (granularity == Days)
                divisor = Time.msPerDay;
            else if (granularity == Weeks)
                divisor = Time.msPerWeek;
     ...
     * will be negative.
     */
    since(earlierDate, granularity = Years) {
     ...
     * negative.
     */
    until(laterDate, granularity = Years) {
    ...
    adjustment(interval, granularity, addition) {
        ...
        if (granularity == Years || granularity == Months) {
            ...
            let multiplier = 0;
            if (granularity == Seconds)
                multiplier = Time.msPerSecond; /** default: Seconds */
            else if (granularity == Minutes)
                multiplier = Time.msPerMinute;
            else if (granularity == Hours)
                multiplier = Time.msPerHour;
            else if (granularity == Days)
                multiplier = Time.msPerDay;
            else if (granularity == Weeks)
                multiplier = Time.msPerWeek;
            interval *= multiplier;
