...
export class Instant extends Date {
    ...
    constructor(initializer = null) {
        let instantComponents = { date: new Date(NaN), precision: FullPrecision, approximation: false };
        if (initializer === null)
            super();
        else if (typeof initializer == 'number')
            super(initializer);
        else if (initializer instanceof Date)
            super(initializer.valueOf());
        else {
    ...
    static parseDateString(dateString) {
        ...
        else if (!InstantString.test(dateString))
            return instantComponents; /** reject invalid dateString */
    ...
    clone() {
        ...
        if (this.precision == FullPrecision)
            clone = new Instant(this.valueOf());
        else {
    ...
    formatted(precision = '') {
        ...
        else if (precision == MonthDayPrecision)
    ...
    adjustment(interval, granularity, addition) {
        ...
        if (granularity == 'Y' || granularity == 'M') { /** Years or Months */
            ...
            if (monthOffset == 1 && Instant.isLeapYear(year))
     ...
     * `instant2`
     */
    static compare(instant1, instant2) {
