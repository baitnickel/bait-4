...
function processTimedEvents(options) {
...
function displayGrid(container, timedEvents, birthday) {
    ...
    for (const timedEvent of timedEvents) {
        ...
        for (let i = 0; i < fields.length; i += 1) {
            ...
            if (i == 0)
                item.className = 'timeline-date';
            else if (i == 1)
                item.className = 'timeline-description';
            else
                item.className = 'timeline-age';
            let value = fields[i];
 ...
 */
function getGrade(birthDate, targetDate) {
    let grade = '';
