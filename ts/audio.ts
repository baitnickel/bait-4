...
for (const playlistFolder of PlaylistFolders) {
	const playlist = await Fetch.json<Playlist>(`${MediaFolders}/${playlistFolder}/_index.json`);
...
const trackPlaying = (audioFile: string) => {
 ...
 */
function displayPlaylistSummaries(playlists: Playlist[]) {
 ...
 */
function getTrackSummary(track: Track, includeTitle = false) {
 ...
 */
function loadPlaylist(playlists: Playlist[], playlistQuery: string) {
	...
	let errorMessage = '';
	if (foundPlaylist === null) errorMessage = `Invalid Playlist ID: ${playlistQuery}.`;
	else if (!foundPlaylist.sequence.length) errorMessage = `Playlist ${foundPlaylist.folder} has no tracks.`;

	if (errorMessage) { TrackListBlock.innerHTML = `<i>${errorMessage}</i>`; }
	return foundPlaylist;
}

// function loadTrackList(tracks: Track[], playlistTracks: string[]) {
function loadTrackList(playlist: Playlist) {
	...
	if (playlist.sequence.length) {
		for (let i = 0; i < playlist.sequence.length; i += 1) {
			...
			if (i == 0) trackElement.classList.add('pad-top');
...
function getTrack(playlist: Playlist, audioFile: string) {
	...
	if (track === undefined) track = defaultTrack;
	return track;
}

function singleTrack(playlist: Playlist, track: Track) {
...
function setHeading(element: HTMLDivElement, text: string, level = 2) {
