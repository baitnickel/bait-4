...
function displayAlbumSummaries(albums: Album[]) {
...
function getTrackSummary(track: Track, includeTitle: boolean = false) {
	/**
	...
	if (includeTitle) noteLines.push(`**${track.title}**`);
	...
	if (track.date) noteLines.push(`Recorded: ${track.date}`);
...
function loadAlbum(albums: Album[], albumQuery: string, tracksQuery: string|null) {
	...
	let errorMessage = '';
	if (!albumFound) errorMessage = `Invalid Album ID: ${albumQuery}.`;
	else if (!requestedTrackIds.length) errorMessage = `Album ${albumQuery} has no tracks.`;
	else if (tracksQuery) {
		...
		for (let queryTrackId of queryTrackIds) {
			if (!requestedTrackIds.includes(queryTrackId)) invalidIds.push(queryTrackId);
		}
		if (invalidIds.length == 1) errorMessage = `Invalid Track ID: ${invalidIds[0]}`;
		else if	(invalidIds.length > 1) errorMessage = `Invalid Track IDs: ${invalidIds.join()}`;
		else requestedTrackIds = queryTrackIds;
	}
	if (errorMessage) {
...
function loadTrackList(tracks: Track[], requestedTrackIds: string[]) {
	...
	if (requestedTrackIds.length) {
		for (let requestedTrackId of requestedTrackIds) {
			...
			if (requestedTrack) selectedTracks.push(requestedTrack);
		}
		...
		for (let selectedTrack of selectedTracks) {
			...
			if (i == 1) trackElement.classList.add('pad-top');
...
function singleTrack(track: Track) {
...
function playTracks(selectedTracks: Track[]) {
	...
	MainAudioElement.addEventListener('play', (e: Event) => {
		let element = e.target as HTMLAudioElement;
