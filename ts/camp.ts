...
type Campsite = {
	category: string; /** not all entries have this property */
	site: string;
	type: string;
	size: string;
	tents: number;
...
function render(page: Page) {
	...
	DB.fetchData('@db/parks.json').then((parks: Park[]) => {
		let mapFile = '';
		let campsites: Campsite[] = [];
		let campgroundComments: string[] = [];
		parksLoop:
		...
		for (let campsite of campsites) {
			let rowElement = document.createElement('tr');
			if (campsite.category) rowElement.classList.add(`${campsite.category}-campsite`);
			let rowItemElement = document.createElement('th');
			...
			rowElement.appendChild(rowItemElement);
			rowItemElement = document.createElement('td');
			if (campsite.tents) rowItemElement.innerText = campsite.tents.toString();
			rowElement.appendChild(rowItemElement);
			rowItemElement = document.createElement('td');
...
function displayReservationTable(tableElement: HTMLTableElement) {
	let siteReservations: SiteReservations = {};
	let beginDate: Date|null = null;
	let endDate: Date|null = null;
	DB.fetchData('@db/park-accounts.json').then((reservationAccounts: ReservationAccounts) => {
		console.log(reservationAccounts);
		DB.fetchData('@db/park-reservations.json').then((reservations: Reservation[]) => {
			/**
			...
			for (let site of sites) {
				...
				for (let reservation in siteReservations[site]) {
					...
					let color = 'lightgray';
					if (reservationAccounts[account]) color = reservationAccounts[account].color;
...
function sortSites(siteReservations: SiteReservations) {
	...
	siteKeys.sort((a, b) => {
		...
		else if (siteReservations[a][0].numericSite < siteReservations[b][0].numericSite) return -1;
		else if (siteReservations[a][0].numericSite > siteReservations[b][0].numericSite) return 1;
		return 0;
	});
	return siteKeys;
