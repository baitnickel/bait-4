...
function newDiagram(division: HTMLDivElement, diagramSize: W.Range) {
	...
	for (let fret = 0; fret < diagram.frets; fret += 1) fretNumbers[fret].innerHTML = `${fret + 1}`;
	for (let string = 0; string < diagram.strings; string += 1) nutMarks[string].innerHTML = 'x';
...
*/
function fingered(id: string, diagram: ChordDiagram) {
...
class ChordDiagram {
	...
	chordName(name: string, verticalAdjustment: number) {
	 ...
	 */
	nutMarks(verticalAdjustment: number, fontSizeAdjustment: number) {
	 ...
	 * top fret).
	 */
	fretNumbers(verticalAdjustment: number, fontSizeAdjustment: number) {
	 ...
	 * clicked.
	 */
	fingerMarks(verticalAdjustment: number, radiusAdjustment: number, ) {
