...
export function render() {
	...
	if (PAGE.local) {
		...
		testers.push( { name: 'Talk Time', function: testTalkTimes } );
...
// 	const gridPoint = new DOMRect(0, 0, fretNumberWidth + fretMargin, nameHeight + nutHeight);
// 	const namePoint = new DOMPoint(gridPoint.x + gridCenter, Math.round(nameHeight / 3 * 2));
...
function testUTC(testOutput: HTMLDivElement) {
...
async function testTalkTimes(testOutput: HTMLDivElement) {
	...
	const audioFiles = await Fetch.apiGet<T.File[]>(uri);
	...
	if (audioFiles === null) console.error(`/media/files API returned no files!`);
		...
		for (const audioFile of audioFiles) {
			if (!T.IsAudioFile(audioFile.base)) continue;
		...
		else console.log('no durations found');
	}
