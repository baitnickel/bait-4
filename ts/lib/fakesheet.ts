 ...
 */
function PitchName(pitch: number, key = 'C') {
	let noteName = '';
...
/**
 * Instruments are keyed by lowercase name (e.g., 'guitar'). Only stringed
 * instruments are supported. Their properties are `frets` (the maximum number
 * of frets on the neck, used to validate notations) and tunings (MIDI pitch
 * numbers for each string, representing standard tuning).
 * 
 * For reference see:
 ...
 */
type InstrumentProperties = { frets: number; tunings: number[] };
const Instruments = new Map<string, InstrumentProperties>();
...
export class Instrument {
	...
	pitches: number[];  /* pitch numbers of the instrument's open strings (0...127) */
	...
	constructor(name: string, alternateTunings: string[] = []) {
		...
		if (alternateTunings.length) this.updatePitches(alternateTunings);
	}
	...
	updatePitches(notes: string[]) {
		for (let i = 0; i < this.pitches.length && i < notes.length; i += 1) {
			...
			if (newIndex >= 0) {
				...
				if (offset >= half) offset -= Notes.length;
				else if (offset < -half) offset += Notes.length;
			}
	...
	setTuning(propertyName: string, values: string[]) {
		...
		if (this.chords.length) {
			...
			for (let note of notes) {
				if (!ValidNote.test(note)) invalidNotes.push(note);
			}
...
export class Chord {
	...
	instrument: Instrument|null; /** Instrument object */
	...
	base: string;                /** base of chord (or key) (e.g., 'Dbm') */
	minor: boolean               /** true if this.base ends with 'm' */
	noteIndices: number[];       /** sequential list of Note indices (e.g., 'Dbm7/Ab' contains 2 indices: [1, 8] (for 'Db' and 'Ab') */
	modifiers: string[];         /** sequential list of modifiers (e.g., 'Dbm7/Ab' contains three modifiers: ['', 'm7/', and ''] */
	...
	*/
	effectiveKey(capo: number) {
		...
		if (noteNumber >= Notes.length) noteNumber -= Notes.length;
	...
	*/
	suggestedCapo(key: Chord, capo: number) {
		...
		if (suggestedCapo < 0) suggestedCapo += Notes.length;
	...
	intervals(noteNames = false) {
		...
		if (this.instrument !== null && this.notation !== null && rootIndex >= 0) {
			...
			for (let instrumentString = 0; instrumentString < this.instrument.strings; instrumentString += 1) {
				if (isNaN(notes[instrumentString][0])) continue; /** ignore "x" strings */
				...
				if (intervalsIndex == 0) {
					...
					if (octave > 0) interval += `'`.repeat(octave); /** number of ticks is distance between octaves */
				}
	 ...
	 */
	intervalPattern(intervals: string[]) {
		let qualifier = '?';
		const pattern = this.uniqueIntervals(intervals).join(' ');
		if (pattern == '1 3 5') qualifier = '';
		else if (pattern == '1 3 5 b7') qualifier = '7';
		else if (pattern == '1 3 5 b7 9') qualifier = '9';
		else if (pattern == '1 3 b7 9') qualifier = '9(no5)';
		else if (pattern == '1 3 5 7') qualifier = 'maj7';
		else if (pattern == '1 3 5 11') qualifier = 'sus4';
		else if (pattern == '1 3 b7 11') qualifier = '11(no5)';
		else if (pattern == '1 3 5 b7 11') qualifier = '11';
		else if (pattern == '1 3 #5') qualifier = 'aug';
		else if (pattern == '1 b3 5') qualifier = 'm';
		else if (pattern == '1 b3 5 b7') qualifier = 'm7';
		else if (pattern == '1 b3 b5 6') qualifier = 'dim';
		else if (pattern == '1 4 5') qualifier = 'sus4';
		else if (pattern == '1 5 b7 11') qualifier = '11';
		else if (pattern == '1 5 9') qualifier = 'sus2';
		else if (pattern == '1 5 11') qualifier = 'sus4';
		return qualifier;
	}
...
class Notation {
	valid: boolean;
	notes: number [][];  /** for each of the instrument's strings, a list of note(s) to be fretted */
	...
	constructor(chordNotation: string, stringCount: number, fretCount: number = 5) {
		this.valid = true;
