...
export class Moment {
	date: Date|null;
	precision: number; /** number of YMD elements provided */
	...
	validDay(year: number, monthOffset: number, day: number) {
		let valid = false;
		let month = monthOffset + 1;
		let leapYear = (year % 4 == 0 && year % 400 != 0);
		if (month == 2) {
			if (day <= 28 || (leapYear && day <= 29)) valid = true;
		}
		else if ([4,6,9,11].includes(month) && day <= 30) valid = true;
		else if (day <= 31) valid = true;
		return valid;
	}
}