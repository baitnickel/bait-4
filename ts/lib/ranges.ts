...
export class NumberRange {
	 ...
	 */
	recalibrate(number: number, otherRange: NumberRange, circular = false) {
		/** constrain number to the limits of its range */
		if (number < otherRange.first) number = otherRange.first;
		else if (number > otherRange.last) number = otherRange.last;
		...
		if (circular && newNumber >= this.size) newNumber = this.last + this.size - newNumber;
		return newNumber;
	}
}