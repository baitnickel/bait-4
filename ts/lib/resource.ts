...
export class Resource {
	url: string;                        /** location */
	id: string;                         /** corresponds to CSS/HTML ID */
	classes: string[];                  /** corresponds to CSS/HTML class(es) */
	options: Map<string, string>;       /** associative array of options */
			...
			for (let word of words) {
				if (word) { /** ignore empty words (products of uncompressed spaces) */
					if (!this.url) {
						...
						if (!this.id) this.id = word.slice(1);
					}
					else if (word.startsWith('.')) this.classes.push(word.slice(1));
					else this.options.set(word, ''); /** attribute without value (boolean) */
				}
			}
		}
	}

	render(type: string, inlineText: string) {
		let html: string;
		let closure: string;
		if (type == 'img') { /** ### IMAGE_TAG and other constants (in markup.ts) should be in a shared module */
			html = `<img src="${this.url}"`;
			if (inlineText) html += ` alt="${inlineText}"`
			closure = '>';
		}
		else { /** type == 'a' */
 ...
 */
export function SmugURI(id: string, size = 'O', type = 'jpg') {
