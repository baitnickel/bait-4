...
export class Widget {
	...
	constructor(element: HTMLElement, labelHTML = '', appendElement = true) {
		...
		if (appendElement) this.label.append(element);
	}
...
export class RadioInput extends Widget {
	...
	constructor(labelHTML: string, groupName: string, checked: boolean, appendElement = false) {
		const element = document.createElement('input');
		super(element, labelHTML, appendElement);
		this.element = element;
...
export class Table {
	...
	headingCells: HTMLTableCellElement[]; /** header cells (client may listen for click events, etc.) */
	headingBaseID: string; /** base ID assigned to heading cells (will become <base>-0, <base>-1 ...) */
	 ...
	 */
	addCell(text: string, attributes: string|string[] = [], html = false) {
	 ...
	 */
	fillTable(table: HTMLTableElement|null = null, html = false) {
		...
		for (const headingValue of this.headingValues) {
			...
			if (html) cell.innerHTML = headingValue;
			else cell.innerText = headingValue;
			row.append(cell);
		}
		table.append(row);
