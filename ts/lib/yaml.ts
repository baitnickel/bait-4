...
export class YAML {
	...
	parse(convertStrings = false) {
		...
		for (const node of this.nodes) {
			...
			if (this.invalidDepth(node, blocks)) continue;
			...
			if (this.typeMismatch(node, blocks)) continue;
			...
			if (node.depth == blocks[i].depth) {
				...
				if (!node.value) parentNode = node;
					...
					if (blocks[i].type == SEQ) blocks[i].data.push(nodeData);
					else blocks[i].data[node.key] = nodeData;
				}
			}
		}
		...
		if (parentNode) {
			if (blocks.length) {
				if (blocks[i].type == SEQ) blocks[i].data.push(null);
				else blocks[i].data[parentNode.key] = null;
			}
		}
	 ...
	 * method).
	 */
	addBlock(node: YAMLNode, blocks: Block[], parent: YAMLNode|null = null) {
		const block: Block = {
		...
		if (parent !== null && parent.depth >= node.depth) block.data = null;
		blocks.push(block)
	}
	 ...
	 */
	rollUpBlocks(blocks: Block[], depth: number = 0) {
		let data: any = null;
		let i = blocks.length - 1; /* index of current block */
		if (blocks.length) {
			while (blocks.length > 1 && blocks[i].parent && depth <= blocks[i].parent!.depth) {
				if (blocks[i].parent!.type == SEQ) blocks[i-1].data.push(blocks[i].data);
				else blocks[i-1].data[blocks[i].parent!.key] = blocks[i].data;
				blocks.pop();
				i = blocks.length - 1;
			}
			data = blocks[i].data;
	...
	yamlNodes() {
		...
		for (let textLine of this.lines) {
			...
			if (!textLine.trim() || textLine.trimEnd() == Separator) continue;
			...
			if (this.missingKey(node)) continue;
			if (this.duplicateKey(node, nodes)) continue;
	 ...
	 */
	convertStringValue(value: string) {
	 ...
	 */
	interpretValue(value: string, convertStrings: boolean) {
		let data: any = null;
		if (value) {
			const matchMap = value.match(/^[\s]*{(.*)}[\s]*$/);
			const matchSequence = value.match(/^[\s]*\[(.*)\][\s]*$/);
			if (matchMap) data = this.flowMap(matchMap[1], convertStrings);
			else if (matchSequence) data = this.flowSequence(matchSequence[1], convertStrings);
			else data = (convertStrings) ? this.convertStringValue(value) : value;
		}
		return data;
	}
	 ...
	 */
	flowMap(expression: string, convertStrings: boolean) {
		const data: any = {};
		...
		for (const keyValue of keyValues) {
			...
			if (value === null) data[key] = value;
				 ...
				 if (convertStrings) data[key] = this.convertStringValue(value);
				 else data[key] = value;
			}
	 ...
	 */
	flowSequence(expression: string, convertStrings: boolean) {
		const data: any = [];
		const values = this.commaSeparatedValues(expression);
		for (const value of values) {
			if (convertStrings) data.push(this.convertStringValue(value.trim()));
			else data.push(value.trim());
		}
		return data;
	}
	 ...
	 */
	duplicateKey(node: YAMLNode, currentNodes: YAMLNode[]) {
		let result = false;
		if (currentNodes.length && node.key != SEQUENCE_KEY) {
			...
			for (let i = currentNodes.length - 1; i >= 0; i -= 1) {
				if (currentNodes[i].depth < node.depth) break;
	 ...
	 */
	invalidDepth(node: YAMLNode, blocks: Block[]) {
		let invalid = true;
		...
		if (invalid) this.recordException(node.line, ERRORS['invalidDepth']);
		return invalid;
	}
	 ...
	 */
	typeMismatch(node: YAMLNode, blocks: Block[]) {
		let result = false;
		let i = blocks.length - 1; /* index of current block */
	 ...
	 */
	recordException(line: number, errorMessage: string) {
