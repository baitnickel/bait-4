...
import { FAKESHEET, FakeSheet, FakeSheetMetadata } from './lib/fakesheet.js'
...
function metadata(documentMetadata: any) {
	let title = '';
	let artist = '';
	...
	let chords: string[] = [];
	if (documentMetadata) {
		if ('title' in documentMetadata) title =  documentMetadata['title'];
		if ('artist' in documentMetadata) artist = documentMetadata['artist'];
		if ('composers' in documentMetadata) {
			if (Array.isArray(documentMetadata['composers'])) composers = documentMetadata['composers'];
			else composers.push(documentMetadata['composers']);
		}
		if ('key' in documentMetadata) key = documentMetadata['key'];
		if ('capo' in documentMetadata) {
			if (!isNaN(documentMetadata['capo'])) capo = Number(documentMetadata['capo']);
		}
		if ('tuning' in documentMetadata) tuning = documentMetadata['tuning'];
		if ('tempo' in documentMetadata) tempo = documentMetadata['tempo'];
		if ('copyright' in documentMetadata) copyright = documentMetadata['copyright'];
		if ('chords' in documentMetadata) {
			if (Array.isArray(documentMetadata['chords'])) chords = documentMetadata['chords'];
			else chords.push(documentMetadata['chords']);
		}
	}
	let fakeSheetMetadata: FakeSheetMetadata = {
		title: title,
		artist: artist,
		composers: composers,
		key: key,
