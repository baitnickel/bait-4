...
const EventOptions: T.EventOptions = {
	source: '',
	from: '',
	until: '',
	keywords: '',
...
function getEvents() {
	Fetch.api<T.Event[]>(`${PAGE.backend}/${ApiRoute}`, Options).then((events) => {
		displayTable(TimelineElement, events);
	});
}

function displayTable(container: HTMLElement, events: T.Event[]|null) {
	container.innerHTML = '';
	...
	if (Options.birthdate) headings.push('Age');
	...
	for (const event of events) {
		...
		if (event.ageGrade) table.addCell(event.ageGrade, 'class:timeline-age');
	}
	table.fillTable();
